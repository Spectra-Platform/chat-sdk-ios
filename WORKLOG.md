...
- 검증 상태: `swift test` 40 tests 통과, `git diff --check` 통과, `git ls-remote`로 `origin/main`과 `refs/tags/v0.2.0` 원격 반영을 확인했다. 임시 SwiftPM package에서 `exact: "0.2.0"` resolution 성공을 확인했다.
